...
- `query_text_filter` — Search queries by name or SQL content (maps to API `query` parameter).
- `team_id` — Filter queries by team.
- `platform_filter` — Filter by scheduled platform: `macos`, `windows`, or `linux`.
- `merge_inherited` — Include global queries when `team_id` is specified (Fleet Premium).
...
### List queries for a specific team with inherited global queries

View all queries for a team including inherited global queries.
...
### Find macOS-specific scheduled queries

Filter queries by platform to find only macOS-targeted queries.
