...
func tableFleetdmHostDetail(ctx context.Context) *plugin.Table {
	return &plugin.Table{
		...
		Columns: []*plugin.Column{
			...
			{Name: "last_mdm_checked_in_at", Type: proto.ColumnType_TIMESTAMP, Hydrate: getHostDetails, Transform: transform.FromField("LastMdmCheckedInAt").Transform(flexibleTimeTransform), Description: "Timestamp when the host last checked in with MDM."},
			{Name: "last_mdm_enrolled_at", Type: proto.ColumnType_TIMESTAMP, Hydrate: getHostDetails, Transform: transform.FromField("LastMdmEnrolledAt").Transform(flexibleTimeTransform), Description: "Timestamp when the host was last enrolled in MDM."},
